pipeline {
    ...
    stages {
        stage('Checkout Code') { // Đã thêm tên stage
            steps {
        ...
        stage("Bien dich va kiem thu") {
            ...
            environment {
                CGO_ENABLED = 0 // Đã sửa tên biến từ CGO_DISABLED thành CGO_ENABLED
                GOOS        = 'linux'
            }
            steps {
                echo 'Đang tải thư viện và biên dịch ứng dụng...'
                sh 'go mod tidy'
                // Build từ gốc, gọi tới file main.go trong cmd/api
                sh 'go build -a -installsuffix cgo -o test-golang ./cmd/api/main.go'
            }
        }
        ...
        stage("Dong goi docker image") {
            steps {
                ...
                // Đã sửa lỗi thiếu dấu $ ở BUILD_NUMBER
    ...
    post {
        always {
            sh 'rm -f .env'
            sh "rm -f ${APP_NAME}"
            cleanWs() // Đã sửa từ clear Ws() thành cleanWs()
        }
        success {
