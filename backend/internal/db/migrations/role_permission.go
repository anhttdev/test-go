...
func SeedRolesAndPermissions(db *gorm.DB) {
	...
	for _, p := range permissions {
		db.FirstOrCreate(&p, model2.Permission{PermissionCode: p.PermissionCode})
	}

	// 2. Định nghĩa 6 Vai trò mặc định và Map quyền luôn cho tụi nó
	var allPerms []model2.Permission
	db.Find(&allPerms)

	// Lọc nhanh danh sách quyền phục vụ gán mặc định
	permMap := make(map[string]model2.Permission)
	for _, p := range allPerms {
		permMap[p.PermissionCode] = p
	}

	// Hàm hỗ trợ gom cụm quyền
	getPerms := func(codes ...string) []model2.Permission {
		var res []model2.Permission
		for _, c := range codes {
