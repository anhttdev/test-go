...
func (uh *UserHandler) CreateUser(ctx *gin.Context) {
	var registerInput dto2.RegisterInput
	...
	if len(duplicateErrors) > 0 {
		ctx.JSON(http.StatusConflict, dto2.Response{
			false,
			"Xung đột dữ liệu",
			http.StatusConflict,
			duplicateErrors,
	...
	if err := uh.repo.Create(&user); err != nil {
		ctx.Error(err)
		ctx.JSON(http.StatusInternalServerError, dto2.NewErrorResponse("Lỗi lưu dữ liệu", 500, nil))
		return
	}
...
func (uh *UserHandler) GetAll(ctx *gin.Context) {
	...
	var request dto2.RequestSearchQuery
...
func (uh *UserHandler) GetUserCursor(ctx *gin.Context) {
	var request dto2.CursorPagingUser
	if err := ctx.ShouldBindQuery(&request); err != nil {
		ctx.Error(err)
		return
	}
	...
	ctx.JSON(http.StatusOK, dto2.UserCursorResponse{
		Success: true,
		Data:    users,
		Paging:  pagingInfo,
	})
