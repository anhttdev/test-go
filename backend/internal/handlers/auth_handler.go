...
type AuthHandler struct {
	AutherService  service2.AuthService
...
func NewAuthHandler(AuthService service2.AuthService, JwtService service2.JwtService, RedisClient *redis.Client, db *gorm.DB, AccountService service2.AccountService) *AuthHandler {
	return &AuthHandler{
...
func (h *AuthHandler) VerifyResetToken(ctx *gin.Context) {
	...
	var resetObj model2.PasswordReset
...
func (h *AuthHandler) ResetPassword(ctx *gin.Context) {
	...
	err = h.db.Model(&model2.Account{}).
		Where("user_id = (?)", h.db.Model(&model2.User{}).Select("id").Where("gmail = ?", resetObj.Email)).
