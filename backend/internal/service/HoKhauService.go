...
type HoKhauService struct {
	...
	hkr repository2.HoKhauRepository // 2. Repo chính của Hộ Khẩu
	ur  repository2.UserRepository   // 3. Repo phụ để check/update User (nếu cần)
}

func NewHoKhauService(
	db *gorm.DB,
	hkr repository2.HoKhauRepository,
...
func (hks *HoKhauService) CreateHoKhau(input dto2.HoKhauInput) (dto2.HoKhauResponse, error) {
	var res dto2.HoKhauResponse
	err := hks.db.Transaction(func(tx *gorm.DB) error {
		var count int64
		tx.Model(&model2.HoKhau{}).Where(
		...
		hkRepoTx := repository2.NewSQLHoKhauRepository(tx)
		usRepoTx := repository2.NewSQLUserRepository(tx)
		...
		res = dto2.ConvertHoKhauToResponse(hokhau)
		return nil
	})
	return res, err
}

func (hks *HoKhauService) TransferMembers(input dto2.TransferMemberInput) error {
	return hks.db.Transaction(func(tx *gorm.DB) error {
		var count int64
		DiaChiCu := input.FromHoKhauID
		tx.Model(&model2.HoKhau{}).Where("id = ?", DiaChiCu).Count(&count)
		if count == 0 {
			return apperr.HouseHoldNotFound
		}

		var validCount int64
		tx.Model(&model2.User{}).
		...
		result := tx.Model(&model2.User{}).Where("id IN ? AND ho_khau_id = ?", input.UserIDs, input.FromHoKhauID).Update("ho_khau_id", input.ToHoKhauID)
		if result.Error != nil {
			return result.Error
		}
...
func (hks *HoKhauService) BurnGetAllHoKhau() ([]model2.HoKhau, error) {
	var danhSachHoKhau []model2.HoKhau
...
func (hks *HoKhauService) GetHoKhauById(id uint) (*model2.HoKhau, error) {
	var count int64
	// 1. Kiểm tra xem cuốn sổ hộ khẩu này có tồn tại dưới DB không
	hks.db.Model(&model2.HoKhau{}).Where("id = ?", id).Count(&count)
	if count == 0 {
		return nil, apperr.HouseHoldNotFound
	}

	var hoKhau model2.HoKhau
...
func (hks *HoKhauService) DeleteHoKhau(hoKhauID uint) error {
	return hks.db.Transaction(func(tx *gorm.DB) error {
		...
		var count int64
		tx.Model(&model2.HoKhau{}).Where("id = ?", hoKhauID).Count(&count)
		if count == 0 {
			return apperr.HouseHoldNotFound
		}
		...
		tx.Model(&model2.User{}).Where("ho_khau_id = ?", hoKhauID).Count(&memberCount)
		if memberCount > 0 {
		...
		result := tx.Where("id = ?", hoKhauID).Delete(&model2.HoKhau{})
		if result.Error != nil {
			return result.Error
		}
