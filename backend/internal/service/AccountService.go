...
func (as *AccountService) FindByUserName(username string, count *int64) error {
	...
	err := as.db.Model(&model2.Account{}).Where("username = ?", username).Count(count).Error
	if err != nil {
		*count = 0
		return err
	}
...
func (as *AccountService) SendResetPassWord(email string) error {
	...
	resetPasswordObj := model2.PasswordReset{
		Email:     email,
		Token:     resetToken,
...
func (as *AccountService) GetAccountRoles(accountID uint) ([]model2.Role, error) {
	var account model2.Account
...
func (as *AccountService) GetProfileByAccountID(accountID uint) (*model2.Account, error) {
	var account model2.Account
